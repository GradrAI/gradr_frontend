...
const Details = () => {
  ...
  return (
    <div className="w-100 h-dvh flex flex-col gap-4 p-8">
      ...
      <div className="w-full h-full flex flex-col gap-8 justify-start border border-border rounded-xl cursor-pointer text-justify p-6 bg-card text-card-foreground overflow-y-auto">
        <div className="flex flex-col justify-start">
          <p className="text-xl m-0 font-bold text-muted-foreground uppercase text-xs tracking-widest mb-2">Overall Score</p>
          <div className="flex items-center justify-start gap-4">
            <p className="m-0 text-3xl font-black text-foreground">
              {isFetchingResult ? (
            ...
            <Button variant="ghost" size="icon" onClick={editScore} className="rounded-full hover:bg-muted">
        ...
        {((latestResult?.result?.results || results) && (latestResult?.result?.results?.length > 0 || results?.length > 0)) && (
          <div className="flex flex-col justify-start gap-4">
            <p className="text-xl m-0 font-bold text-muted-foreground uppercase text-xs tracking-widest border-b pb-2">Question Breakdown</p>
            <div className="grid grid-cols-1 gap-4">
              {(latestResult?.result?.results || results).map((res: any, index: number) => (
                <div key={index} className="p-4 bg-muted/50 dark:bg-zinc-900/50 rounded-xl border border-border dark:border-zinc-800 hover:border-primary/20 transition-colors shadow-sm">
                  <div className="flex justify-between items-center mb-3">
                    <span className="font-bold text-foreground text-sm">{res.questionId || `Question ${index + 1}`}</span>
                  ...
                  <div className="space-y-2">
                    <p className="text-sm leading-relaxed"><span className="font-bold text-muted-foreground text-[10px] uppercase block mb-1">AI Explanation</span>{res.explanation}</p>
                    {res.feedback && (
                      <p className="text-sm leading-relaxed italic border-l-2 border-green-500 pl-3"><span className="font-bold text-muted-foreground text-[10px] uppercase block not-italic mb-1">AI Feedback</span>{res.feedback}</p>
                    )}
                  </div>
                </div>
              ))}
        ...
        <div className="flex flex-col justify-start">
          <p className="text-xl m-0 font-bold text-muted-foreground uppercase text-xs tracking-widest border-b pb-2 mb-4">Summary & Feedback</p>
          <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
            <div className="space-y-2">
              <p className="text-sm font-bold text-muted-foreground uppercase tracking-tighter">AI Justification</p>
              <p className="leading-relaxed text-foreground/90 dark:text-zinc-300">{latestResult?.result?.explanation || explanation}</p>
            </div>
            <div className="space-y-2">
              <p className="text-sm font-bold text-muted-foreground uppercase tracking-tighter">AI General Feedback</p>
              <p className="leading-relaxed text-foreground/90 dark:text-zinc-300 italic">{latestResult?.result?.feedback || feedback}</p>
            </div>
          </div>
        </div>
      </div>
